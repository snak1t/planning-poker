...
module.exports = io => {
    ...
    io.sockets.on('connection', socket => {
        ...
        socket.on('action', ({ type, payload }) => {
            switch (type) {
                ...
                case socketsType.EMIT_ADD_NEW_STORY:
                    return fastForwardWithType(socketsType.BROADCAST_ADD_NEW_STORY, payload);
                case socketsType.EMIT_UPDATE_STORY:
                    return fastForwardWithType(socketsType.BROADCAST_UPDATE_STORY, payload);
                case socketsType.EMIT_DELETED_STORY:
                    return fastForwardWithType(socketsType.BROADCAST_DELETED_STORY, payload);
                default:
                    return;
            }
        });
        ...
        const addToRoom = data => {
            ...
            socket.join(roomID);
            subscription = rooms.subscribe(roomID, data => broadcastSyncGameSession(roomID, data));
        ...
        const dropScores = (roomID, rooms) => rooms.get(roomID).scores.map(p => Object.assign({}, p, { score: null }));
